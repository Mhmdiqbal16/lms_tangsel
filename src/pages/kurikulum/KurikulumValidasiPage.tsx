...
export function KurikulumValidasiPage() {
  ...
  const rows: ValidationRow[] = learningMaterials
    .map((material) => {
  ...
  const executeValidate = async (row: ValidationRow, status: 'Valid' | 'Ditolak') => {
    const pendingKey = `${row.sourceId}-${status}`;
    setPendingAction(pendingKey);
    const result = await setMaterialValidation(row.sourceId, status);
    setPendingAction(null);

    setMessage({
      tone: result.success ? 'success' : 'warning',
      text: result.success
        ? `Materi guru dari ${row.actorName} telah diubah menjadi status ${status}.`
        : result.message,
    });
  ...
  const validationConfirmationLoading = validationConfirmation
    ? pendingAction === `${validationConfirmation.row.sourceId}-${validationConfirmation.status}`
    : false;
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Validasi Data"
        description="Kurikulum memvalidasi materi pembelajaran yang dikirim guru."
      />
        ...
        <div className="max-h-[560px] overflow-auto">
          <table className="min-w-full divide-y divide-brand-100">
            <thead className="bg-brand-50/70">
              <tr>
                {['Jenis Data', 'Guru', 'Tanggal', 'Status Validasi', 'Aksi'].map((header) => (
                  <th
                    key={header}
                ...
                <tr key={row.id} className="align-top hover:bg-brand-50/40">
                  ...
                  <td className="px-4 py-4 text-sm text-slate-700">
                    <div className="flex flex-wrap gap-2">
                      ...
                      >
                        {pendingAction === `${row.sourceId}-Valid` ? 'Menyimpan...' : 'Validasi'}
                      </button>
                      <button
                        type="button"
                      ...
                      >
                        {pendingAction === `${row.sourceId}-Ditolak` ? 'Menyimpan...' : 'Tolak'}
                      </button>
                      <button
                        type="button"
      ...
      <ConfirmDialog
        ...
        title={
          validationConfirmation?.status === 'Valid'
            ? 'Validasi materi guru?'
            : 'Tolak materi guru?'
        }
        description={
          validationConfirmation
            ? `Materi "${validationConfirmation.row.title}" dari ${validationConfirmation.row.actorName} akan diubah menjadi status ${validationConfirmation.status}.`
